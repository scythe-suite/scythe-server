...
docker run --name portainer -v "$(pwd)/var/portainer":/data -p 9000:9000 -v /var/run/docker.sock:/var/run/docker.sock -d portainer/portainer --no-auth -H unix:///var/run/docker.sock
